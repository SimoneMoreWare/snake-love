...
import { createSoundToggleButton, playSound } from './audioController.mjs' // Importa il controller audio
...
export class Game {
  ...
  start() {
    ...
    // Aggiungi il pulsante per il controllo dell'audio
            ...
            <div class="start-container">
                ...
                <div class="start-instructions">
                    ...
                    <ul>
                        ...
                        <li>🔊 Pulsante audio per attivare/disattivare il suono</li>
                    </ul>
                </div>
            </div>
        `
  ...
  createPauseOverlay() {
    ...
    this.pauseOverlay.innerHTML = `<div class="pause-container">
                <h2>PAUSA</h2>
  ...
  updateDirectionWithJoystick() {
    if (Math.abs(this.joystick.value.x) > Math.abs(this.joystick.value.y)) {
      if (this.joystick.value.x > SENSIBILITY)
        this.snake.changeDirection('RIGHT')
      else if (this.joystick.value.x < -SENSIBILITY)
        this.snake.changeDirection('LEFT')
    } else {
      if (this.joystick.value.y > SENSIBILITY)
        this.snake.changeDirection('DOWN')
      else if (this.joystick.value.y < -SENSIBILITY)
